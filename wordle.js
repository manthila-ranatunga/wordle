var height = 6; // number of guesses (can change)
var width = 5; // length of word (can change)
...
// 10 letter words
// const wordList = [
//     "confession","vegetarian","government","production","correction","conspiracy",
//     "relinquish","commission","domination","appreciate","conception","protection",
//     "convulsion","hemisphere","stereotype","productive","reflection","deficiency",
//     "hypnothize","illustrate"
// ];
...
function processInput(e) {
    ...
    if (!gameOver && row == height) { // case where all 6 attempts are used
        gameOver = true;
...
function update() {
    ...
    // string up the guesses into word
    for (let c = 0; c < width; c++) {
    ...
    for (let c = 0; c < width; c++) {
        ...
        if (correct == width) {
            let tries = row + 1;
            document.getElementById("answer").innerText = ("Solved in " + tries + " tries.");
            gameOver = True;
        }
